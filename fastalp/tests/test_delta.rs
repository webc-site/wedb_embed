...
fn test_linear_ramp_constant_delta() -> Result<()> {
  ...
  // 必须自动识别并采用 Delta 模式
  assert_eq!(compressed[0], TYPE_F64_DELTA);
...
fn test_f32_delta_roundtrip() -> Result<()> {
  ...
  let compressed = compress_delta(&data);
  assert!(
    compressed[0] == TYPE_F32_DELTA || compressed[0] == TYPE_F32_DEC_DELTA,
...
fn test_decimal_division_mode_exact_match() -> Result<()> {
  ...
  let compressed = compress(&data);
  assert!(
    compressed[0] == TYPE_F64_DEC || compressed[0] == TYPE_F64_DEC_DELTA,
